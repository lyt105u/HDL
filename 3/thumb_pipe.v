...
// Internal Encoding for Thumb Instructions Supported in "thumb_pipe.v"
...
/////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////
// MODULE: Pipelined CPU for THUMB microprocessor: thumb_pipe.v
// Author: Sunggu Lee
...
module IF (
  input   wire                    reset_n,
  input   wire                    clk,
  input   wire  [`HWORD_SIZE-1:0] instruction,  // current instruction
  input   wire  [`WORD_SIZE-1:0]  branch_target, // branch target address
  input   wire                    branch_taken, // indicates a branch has been taken
  output  reg   [`WORD_SIZE-1:0]  PC,
  output  reg                     read_instruction_n,  // enable read from instruction mem
...
module EX (
  ...
  input   wire  [`WORD_SIZE-1:0]  ID_Rd,
  input   wire  [`WORD_SIZE-1:0]  ID_Rm_Rs,
  input   wire  [`WORD_SIZE-1:0]  ID_Rn,
  input   wire  [10:0]            ID_imm_offset,
  input   wire  [`WORD_SIZE-1:0]  ID_PC,
  input   wire  [3:0]             ID_cond,
  input   wire  [2:0]             ID_Rd_code,
  ...
  always @* begin
    case (ID_cond)  // codes in Table 3-1 of [ARM 2000]
  ...
  always @(negedge reset_n or posedge clk) begin
    if (~reset_n) begin
      ...
      // EX_opcode <= 6'b000000;
      // ALU_out <= 33'b00000000000000000000000000000000;
      // EX_ALU_out <= 32'h00000000;
      // SP <= 32'h00000000;
...
module WB (
  ...
  input   wire  [7:0]             EX_imm_offset,
  output  reg   [255:0]           R_flat
);
  integer wb_i, found_wb_i, i;
  reg [`WORD_SIZE-1:0] R [`REG_FILE_SIZE-1:0];
  always @(negedge reset_n or posedge clk) begin
    if (~reset_n)
      ...
      case (EX_opcode)
        ...
        `LDRSH: begin
          if (data[`HWORD_SIZE-1]) begin
            R[EX_Rd_code] <= {16'hffff, data[`HWORD_SIZE-1:0]}; // 16 = `HWORD_SIZE
          end else begin
            R[EX_Rd_code] <= {16'h0000, data[`HWORD_SIZE-1:0]}; // 16 = `HWORD_SIZE
          end
        end
        `POP_R0: begin
          found_wb_i = 0;
...
// MODULE DECLARATION
module thumb_pipe (read_instruction_n, instruction_address,
  ...
  wire read_data_n, write_data_n;
  wire read_instruction_n;
  ...
  wire [`WORD_SIZE-1:0] LR;        // link register = R14
  wire [`WORD_SIZE-1:0] SP;        // stack pointer = R13
  ...
  wire [`WORD_SIZE:0] ALU_out;     // ALU output
  ...
  IF step1(reset_n, clk, instruction, branch_target, branch_taken, PC, read_instruction_n, IF_IR, IF_PC);
  ID step2(clk, IF_IR, R_flat, branch_taken, IF_PC, ID_opcode, ID_imm_offset, ID_Rn, ID_Rd, ID_Rd_code, ID_Rm_Rs, ID_cond, ID_PC);
  EX step3(reset_n, clk, R_flat, ID_opcode, ID_Rd, ID_Rm_Rs, ID_Rn, ID_imm_offset, ID_PC, ID_cond, ID_Rd_code, branch_taken, read_data_n, write_data_n, EX_opcode, ALU_out, EX_ALU_out, SP, branch_target, LR, data_address, DR, EX_Rd_code, EX_imm_offset, Z_Flag, V_Flag, N_Flag, C_Flag);
  WB step4(reset_n, clk, EX_opcode, EX_Rd_code, EX_ALU_out, data, EX_imm_offset, R_flat);
